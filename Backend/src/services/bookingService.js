...
export const getMyBookings = async (userId) => {
    return await Booking.find({ user: userId }).populate("showtime").sort({ createdAt: -1 });
};

export const getAllBookings = async (filters = {}) => {
    return await Booking.find(filters).populate("user", "name email").populate("showtime").sort({ createdAt: -1 });
};
